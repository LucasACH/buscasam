...
gated by require_docente. Missing/author-soft-deleted report → uniform 404;
...
async def _report(session, doc_id: int) -> int:
...
async def _hidden_at(session, doc_id: int):
    return (
        await session.execute(
...
async def test_hide_returns_204_and_stamps_column(client, session):
    doc_id = await make_document(session, visibility="publico")
    report_id = await _report(session, doc_id)
    ...
    assert await _hidden_at(session, doc_id) is not None


async def test_unhide_returns_204_and_clears_column(client, session):
    doc_id = await make_document(session, visibility="publico", moderation_hidden=True)
    report_id = await _report(session, doc_id)
    cookie = await _docente_cookie(session)

    r = await client.post(
        f"/api/moderation/reports/{report_id}/unhide",
        json={},
        headers=_headers(cookie),
    )

    assert r.status_code == 204
    assert await _hidden_at(session, doc_id) is None


async def test_dismiss_returns_204(client, session):
    doc_id = await make_document(session, visibility="publico")
    report_id = await _report(session, doc_id)
    cookie = await _docente_cookie(session)

    r = await client.post(
        f"/api/moderation/reports/{report_id}/dismiss",
        json={},
        headers=_headers(cookie),
    )
