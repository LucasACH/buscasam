...
async def _file_report(
    session, doc_id: int, reporter: int, *, status: str = "open"
) -> int:
    return (
...
async def test_rehide_reunhide_leaves_no_residue_beyond_audit_log(session):
    ...
    await hide(
        session, ctx, await _file_report(session, doc_id, await make_user(session)), "x"
    )
    await unhide(
        session, ctx, await _file_report(session, doc_id, await make_user(session))
    )
    await hide(
        session, ctx, await _file_report(session, doc_id, await make_user(session)), "y"
    )
