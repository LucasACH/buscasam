...
- **OCR test fails with `SubprocessOutputError` / `fopenReadStream: failed to open locally`** (`tests/integration/test_jobs_ocr_index.py`): leptonica 1.87.0 (Homebrew) can't `fopen` files through the `/tmp` → `/private/tmp` symlink. If `TMPDIR` points at a `/tmp/...` path, ocrmypdf hands tesseract `/tmp/...` paths and OCR aborts. Run the suite with the resolved path: `TMPDIR=/private/tmp uv run pytest`. Linux/CI is unaffected (no `/tmp` symlink). The test self-skips when tesseract+`spa`/`eng` tessdata are absent, so this only bites macOS with the OCR toolchain installed.
