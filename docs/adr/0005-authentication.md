...
17. Email allowlist override (escape hatch for demo / non-UNSAM accounts that must moderate): `BUSCASAM_DOCENTE_EMAIL_ALLOWLIST` is a comma-separated set of exact emails granted `docente` regardless of `hd`. Checked in `role_from_claims` after the `email_verified` gate and before `ROLE_BY_HD`, so it loosens only the domain map — `email_verified != True` is still rejected. Empty by default (no-op). Listed emails should be kept to the minimum and removed when no longer needed; each is a deliberate exception to the `hd`-is-the-sole-trust-boundary rule in item 2.
