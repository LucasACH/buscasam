...
5. Retry policy:
   ...
   `mark_failed` also classifies the failure for the author-facing retry: `index_error_kind='file'` (`corrupted:`, `ocr_failed:` — the upload itself is bad, only a new file helps) vs `'system'` (`ocr_unavailable:`, `exhausted retries:` — our-side fault), and stamps `index_failed_at`. A `'system'` failure is retriable by the author through `core/documents.retry_indexing` (`POST /documents/{id}/retry-indexing`): it resets the never-published failed version to `'pending'`, clears the failure stamps, and re-enqueues `index_document` through the same transactional defer (§1), so the retried job gets a fresh automatic-retry budget from this table. Guards, each → 409: `'system'` kind only, a 5-minute cooldown from `index_failed_at` (`RETRY_COOLDOWN` — the automatic backoff above already burned ~3 minutes, an instant manual retry against a still-down dependency just burns another round), and at most 3 manual retries per version (`index_retry_count` / `MAX_INDEX_RETRIES` — an outage that survives 3 manual rounds needs an operator, not more clicks). The `processing_failed` notification's unique `event_key` means a retry that re-fails the same version cannot re-notify.
