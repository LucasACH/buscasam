...
12. Relevance floor: candidate is displayed when it has a lexical hit or its best semantic cosine similarity meets a launch-calibrated `MIN_SEMANTIC_SIMILARITY` setting. A pure-semantic candidate (no lexical hit) must *additionally* share trigram signal with the query above `SEMANTIC_ONLY_TRGM_THRESHOLD` (lexical grounding) — this guards against `multilingual-e5` short-query cosine inflation surfacing off-topic docs; set the threshold to 0 to revert to floor-only. Calibration fixture (`fixtures/relevance_queries.py`) and harness (`scripts/calibrate_floor.py`) plus the selected value are committed before launch. Below-floor or ungrounded candidates are not results.
