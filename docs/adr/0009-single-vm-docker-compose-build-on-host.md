...
5. **Healthchecks and `depends_on` conditions.**
   ...
   - `nginx`, `worker_default`, `worker_ocr`, `backup`: no healthcheck. Workers explicitly skipped per ADR-0008 §9 (operator observability is ADR-0010); nginx and backup rely on `restart: unless-stopped`.
   - `depends_on`:
...
13. **Schema apply: one explicit step, idempotent both sides.** The `migrate` one-shot runs `alembic upgrade head && procrastinate schema --apply` (closes ADR-0008 §10). The very first Alembic migration in repo history executes `CREATE EXTENSION IF NOT EXISTS vector;`, `CREATE EXTENSION IF NOT EXISTS unaccent;`, `CREATE EXTENSION IF NOT EXISTS ltree;`, and the `CREATE TEXT SEARCH CONFIGURATION es_unaccent …` from ADR-0001 §8 — so extension provisioning and the FTS config are part of the regular migration stream, not a separate manual step. No other Postgres extensions are provisioned at MVP; the search design in ADR-0001 commits to `tsvector` + `ts_rank_cd`, not `pg_trgm` similarity.
...
- **Rate limiting is a coarse first cut.** The §16 starting values are not measured against actual traffic; the auth zone is generous enough to never block a real user, the guest zone may rate-limit aggressive academic crawlers. Recalibration depends on ADR-0010's metrics; a tuning pass after the first month of prod is the expected baseline.
- **Observability is the most-deferred load-bearing concern in this stack.** ADR-0010 inherits a queue depth dashboard need (ADR-0008 §9), several named metrics (ADR-0002 §8, ADR-0007 §4–5, ADR-0008 §4), and the question of log aggregation. The compose graph is structured so that a metrics-emitter sidecar (Prometheus node_exporter + a postgres_exporter, or an OpenTelemetry collector) can be added as a new service without restructuring existing ones.
