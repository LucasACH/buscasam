...
6. Configuration. Four `pydantic-settings` fields, all `BUSCASAM_`-prefixed:
   ...
   `metadata_llm_model` must match the model pulled by the inference host (`infra/terraform/modules/metadata-llm/variables.tf:model`). Dev may point `metadata_llm_url` at a local `ollama serve` with a smaller model (`docs/local-dev.md`); the value is operator-chosen, not pinned.
