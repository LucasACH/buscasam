...
5. **Worker / API code sharing.** API and worker processes import from a single `core/` package (models, `search_query`, `embed`, schemas, settings). The queue (ADR #8) is the only seam; both processes are deployed from the same image / virtualenv and must move together.
...
9. **OpenAPI auto-published.** FastAPI's generated schema is the API contract; the frontend (ADR #4) consumes it. No hand-maintained spec.
...
- **One deploy unit, two processes.** API and worker share code, so they must release together — a worker that drifts behind the API may interpret payloads with a stale schema. Acceptable on the single-VM topology (ADR #9 territory); revisit if processes are ever split across hosts.
