...
**Interface:** `search.run(filters, user_ctx, embedding: halfvec | None) -> Results` (ADR-0003 §3). Inputs: validated filters (q, area_path or None, tipo[], desde, hasta, orden, pagina), the requester's `UserCtx`, and an optional query embedding. Calls `document_access.readable_where("d", user_ctx)` itself — the access predicate does not cross the module boundary as a parameter. Returns `Results { rows: list[ResultRow], total: int }` where each row carries `doc_id, titulo, fecha, area_path, tipo, abstract, snippet, visibility`. Invariants: only `chunks.is_current=true` may contribute lexical matches, semantic matches, or query snippets; 10 rows per page; `pagina` capped at 20 under `orden=relevancia`; uncapped under `orden=recientes`; `embedding=None` ⇒ lexical-only (RRF tolerates empty semantic side); below-`MIN_SEMANTIC_SIMILARITY` rows excluded unless they have a lexical hit; pure-semantic rows (no lexical hit) additionally require trigram grounding against the query above `SEMANTIC_ONLY_TRGM_THRESHOLD`; `total` is `200+` when the fused set saturates under relevance, exact under recientes. Errors: SQLAlchemy/asyncpg errors propagate; no business exceptions.

**Responsibilities:** Hybrid SQL — lexical CTE (`es_unaccent`, `ts_rank_cd`, `ts_headline` for snippet), semantic CTE (pgvector cosine, HNSW iterative scan), RRF fusion, chunk→doc aggregation via `MAX(score)`, current-version eligibility, top-200 cap, área (`ltree <@`), tipo (`IN`), fecha (year range), `orden=recientes` partial-btree path. Owns snippet generation: `ts_headline` on current lexical rows, `LEFT(abstract, 200)` for pure-semantic rows. A staged indexed replacement remains absent from lexical, hybrid, and `recientes&q=` results until `core/documents.publish` flips its chunks current.

**Internal seams:** `_lexical_candidates_ctes(where, filter_clauses, cap)` produces the `lex_scored`/`lex_best`/`lex_ranked` CTE chain (best-matching current chunk per readable doc, optional top-N cap) used identically by `_run_recientes`, `_run_lexical`, and the lex side of `_run_hybrid`. The semantic CTE applies the same `is_current` eligibility before vector ordering. `_headline_expr(body_col)` produces the `ts_headline` SQL expression. These are private to the module — the external interface is unchanged.

**Seams:** None at the external retrieval interface. `core/document_access.readable_where` supplies requester-aware readable policy; `core/search_query` adds current-version chunk eligibility before ranking. Score normalization is locked at RRF only.
