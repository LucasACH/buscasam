...
**Invitado** (guest):
A visitor without an UNSAM Google account. Browses and downloads `público` documents; cannot publish, comment, favourite, or report.
_Avoid_: external user, anonymous (use *invitado* even in code paths).

**Estudiante**:
An UNSAM Google account with `hd = estudiantes.unsam.edu.ar`. Can publish, comment, favourite, report.

**Docente**:
An UNSAM Google account with `hd = unsam.edu.ar`. Same capabilities as **Estudiante** plus moderation (resolve reports, hide content). Treated as "trusted UNSAM staff", not strictly "teaching faculty" — admin and library staff also receive this domain and get the same powers by design.
...
**Visibilidad**:
A per-document setting controlling who sees it. Three values: `público` (everyone including **Invitados**), `interno` (any signed-in UNSAM user — **Estudiante** or **Docente**), `privado` (authors only). Enforced exclusively by the SQL chokepoint (ADR-0001 §9).
...
- A document's **Visibilidad** + the requester's role together decide access; the predicate lives in one SQL builder.

## Example dialogue

> **Dev:** "If a librarian with `@unsam.edu.ar` reports a comment, who can resolve it?"
